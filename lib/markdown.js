...
Markdown.dialects.NAMP.block[ "Note: " ] = function note_maker( block, next ) {
  ...
  var note = [ "div", {class: "alert-message block-message warning"} ];
  note.push( [ "div" ].concat(this.processInline(  m[1].replace( /(\n)\s+/, "$1" ) ) ) );

  return [ note ];
}

Markdown.dialects.NAMP.block[ "Warning: " ] = function note_maker( block, next ) {
  var m = block.match(/Warning: (.+)/);

  if ( !m ) return undefined;

  var note = [ "div", {class: "alert-message block-message error"} ];
  note.push( [ "div" ].concat(this.processInline(  m[1].replace( /(\n)\s+/, "$1" ) ) ) );

...
var highlight = true; // always highlight by default
var conref = false; // don't conref by default
 ...
 **/
expose.parse = function( source, dialect, options) {
...
expose.toHTML = function toHTML( source , options ) {
  ...
  var documentMeta = input[1]; // input gets consumed below, so save the potential meta info

  var htmlOutput = expose.renderJsonML( input );

  if (!documentMetaExists)
    return htmlOutput;
  else
    return [ htmlOutput, documentMeta ];
}

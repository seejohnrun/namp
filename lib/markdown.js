...
var conrefEnabled = false; // don't conref by default
...
var Markdown = expose.Markdown = function Markdown(dialect) {
  switch (typeof dialect) {
    case "undefined":
      this.dialect = Markdown.dialects.NAMP;
      break;
    case "object":
 ...
 **/
expose.parse = function( source, options) {
  // dialect will default
  var md = new Markdown( "NAMP" );
 ...
 **/
expose.toHTML = function toHTML( sourceFile , options ) {
  var dialect = "NAMP"; // default to this, please 
  var htmlOutput;
  var documentMeta;
  ...
  if (options)
  {
    ...
    if (options.conref === true)
      conrefEnabled = true;
  }

  if (conrefEnabled) {
    markdown_conrefs.init(sourceFile);
    var source = fs.readFileSync(sourceFile, "utf8");
    source = markdown_conrefs.replaceConref(source);
  }

  var input = expose.toHTMLTree( source , dialect , options );
  documentMeta = input[1]; // input gets consumed below, so save the potential meta info

  htmlOutput = expose.renderJsonML( input );

  documentMeta = documentMetaExists ? documentMeta : undefined;

  return { html: htmlOutput, metadata: documentMeta };
}
...
expose.toHTMLTree = function toHTMLTree( input, dialect , options ) {
  ...
  if ( typeof input ==="string" ) input = this.parse( input, options );
...
Markdown.prototype.split_blocks = function splitBlocks( input, startLine ) {
  ...
  if ( ( m = /^(\s*\n)/.exec(input) ) != null ) {
    ...
    line_no += helpers.count_lines( m[0] );
...
Markdown.prototype.processBlock = function processBlock( block, next ) {
  ...
  for ( var i = 0; i < ord.length; i++ ) {
    ...
    if ( res ) {
      ...
      if ( !helpers.isArray(res) || ( res.length > 0 && !( helpers.isArray(res[0]) ) ) )
...
Markdown.dialects.GFM.block.code_fence = function code_fence( block, next ) {
  var ret = [],
      //re = /^(?: {0,3}\t| {4})(.*)\n?/,
      re_begin = /^(?:```([^\n\s`]+)?)\s*/,
      re_end = /^\n?```\s*$/;
    ...
    for (var line_no=0; line_no < lines.length; line_no++) {
      ...
      if (m) {
        ...
        if (line_no + 1 < lines.length) {
          next.unshift( mk_block(lines.slice(line_no + 1).join(""), block.trailing) );
        }
        break block_search;
      }
  ...
  if (code_type !== undefined) {
    return [ [ "code_block", { "class": code_type, "lang": code_type }, ret.join("\n") ] ];
  } else {
    return [ [ "code_block", ret.join("\n") ] ];
  }
};

Markdown.dialects.GFM.inline[ "  \n" ] = function line_break( text, matches, out ) {
  return [ 2, [ "linebreak" ] ];
}
...
// NAMP does all the above...plus more!
Markdown.dialects.NAMP = Markdown.subclassDialect( Markdown.dialects.GFM );

// marked attributes; Maruku metadata only applies to Markdown inline elements, like bold, or italic. 
// This system allows you to conref even plain-text.
Markdown.dialects.NAMP.inline[ "[" ] = function scoped_inline_meta( text, matches, out ) {
  // match a meta hash
  var m = text.match( /\[(.+?)\]\{:\s*((?:\\\}|[^\}])*)\s*\}/ );

  // no match--perhaps a link?
  if ( !m ) { 
...
Markdown.dialects.NAMP.block[ "Note: " ] = function note_maker( block, next ) {
  ...
  var note = [ "div", {class: "alert alert-success"} ];
  note.push( [ "h4", {class: "alert-heading"}].concat("Note: ") );
  note.push( [ "div" ].concat(this.processInline(  m[1].replace( /(\n)\s+/, "$1" ) ) ) );
...
Markdown.dialects.NAMP.block[ "Tip: " ] = function tip_maker( block, next ) {
  ...
  var note = [ "div", {class: "alert alert-info"} ];
  note.push( [ "h4", {class: "alert-heading"}].concat("Tip: ") );
  note.push( [ "div" ].concat(this.processInline(  m[1].replace( /(\n)\s+/, "$1" ) ) ) );
...
Markdown.dialects.NAMP.block[ "Warning: " ] = function warning_maker( block, next ) {
  ...
  var note = [ "div", {class: "alert alert-block"} ];
  note.push( [ "h4", {class: "alert-heading"}].concat("Warning: ") );
  note.push( [ "div" ].concat(this.processInline(  m[1].replace( /(\n)\s+/, "$1" ) ) ) );
...
Markdown.buildInlinePatterns( Markdown.dialects.NAMP.inline );
...
function render_tree( jsonml ) {
  if (highlightEnabled) {
    if (jsonml[0] == "code") 
    {
        if (jsonml[2] !== undefined) {
          return hl.highlight(jsonml[2].join(""), jsonml[1]).value;
        }
        else
          return hl.highlightAuto(jsonml[1]).value;  
    }
  }

  // hack to fix bug with above
